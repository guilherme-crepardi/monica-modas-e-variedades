(function () {
  ...
  function imagemProduto(p) {
    ...
    const fundo = cores[p.categoria] || "#f2f2f4";
    const svg =
      ...
      '<path d="' + iconesCategoria[p.categoria] + '"/></g>' +
  ...
  function linkWhatsApp(p) {
    const msg =
      ...
      "* (" + nomesCategoria[p.categoria] + ") no valor de " +
  ...
  function tituloCategoria(cat) {
    return nomesCategoria[cat] || "Destaques";
  }
    ...
    if (buscaAtual) {
      ...
      lista = lista.filter(function (p) {
        return p.nome.toLowerCase().indexOf(b) !== -1 ||
          nomesCategoria[p.categoria].toLowerCase().indexOf(b) !== -1;
      });
    }
    return lista;
  }
  ...
  function renderizar() {
    ...
    titleEl.textContent = buscaAtual ? "Resultados da busca" : tituloCategoria(filtroAtual);
    ...
    lista.forEach(function (p, i) {
      ...
      card.innerHTML =
        ...
        '<div class="product-body">' +
          '<span class="product-cat">' + nomesCategoria[p.categoria] + "</span>" +
    ...
    grids.querySelectorAll(".btn-detail").forEach(function (btn) {
      btn.addEventListener("click", function () { abrirModal(Number(btn.dataset.id)); });
    });
  }
  ...
  function setarFiltro(cat, origem) {
    filtroAtual = cat;
    buscaAtual = "";
    searchInput.value = "";
  ...
  document.querySelectorAll(".nav-link, .filter-chip, .category-card").forEach(function (btn) {
    btn.addEventListener("click", function () {
      setarFiltro(btn.dataset.categoria, btn.classList.contains("nav-link") ? "nav" : "");
    });
  });
  ...
  const waMsgGeral =
    "Olá! Vim pelo site da Monica Modas e Variedades e quero saber mais.";
  document.querySelectorAll("[id^='link-whatsapp']").forEach(function (a) {
    ...
    if (a.id === "link-whatsapp-social" || a.id === "link-whatsapp-footer") {
      msg = waMsgGeral;
    } else if (a.id === "link-whatsapp-cta") {
      msg = "Olá! Quero usar o cupom MONICA10 e ganhar 10% OFF na primeira compra.";
    }
  ...
  floatBtn.href = waBase + "?text=" + encodeURIComponent(waMsgGeral);
